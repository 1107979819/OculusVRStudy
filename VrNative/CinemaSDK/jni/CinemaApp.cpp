 ...
 *
 */
void CinemaApp::OneTimeInit( const char * launchIntent ) {
	...
	ShaderMgr.OneTimeInit( launchIntent );
	ModelMgr.OneTimeInit( launchIntent );
	SceneMgr.OneTimeInit( launchIntent );
	MovieMgr.OneTimeInit( launchIntent );
	MoviePlayer.OneTimeInit( launchIntent );
	MovieSelectionMenu.OneTimeInit( launchIntent );
	TheaterSelectionMenu.OneTimeInit( launchIntent );
	ResumeMovieMenu.OneTimeInit( launchIntent );
	...
	LOG( "CinemaApp::OneTimeInit: %3.1f seconds", TimeInSeconds() - StartTime );
}
...
const char * CinemaApp::RetailDir( const char *dir ) const
{
	...
	StringUtils::SPrintf( subDir, "/sdcard/RetailMedia/%s", dir );
	return subDir;
}
...
{
	static char subDir[ 256 ];
	StringUtils::SPrintf( subDir, "/storage/extSdCard/RetailMedia/%s", dir );
	return subDir;
}
...
void CinemaApp::StartMoviePlayback()
{
	...
	{
		MovieFinishedPlaying = false;
		Native::StartMovie( app, CurrentMovie->Filename.ToCStr(), CurrentMovie->Title.ToCStr(), ShouldResumeMovie, CurrentMovie->IsEncrypted );
	}
}
...
{
	InLobby = false;
	MovieSelectionMenu.SetError( Strings::Error_UnableToPlayMovie, false, true );
	ViewMgr.OpenView( MovieSelectionMenu );
}
