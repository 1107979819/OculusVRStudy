...
static const char* movieUiVertexShaderSrc =
	...
	"   oTexCoord1 = vec2( Texm2 * vec4(TexCoord,1,1) );\n"
	"   oColor = UniformColor;\n"
	"}\n";


// The uniformColor.w will fade the UI independently of the screen
const char* movieExternalUiFragmentShaderSource =
	...
	"void main()\n"
	"{\n"
	" 	lowp vec4 uiColor = texture2D( Texture2, oTexCoord1 );\n"
	"	lowp vec4 movieColor = texture2D( Texture0, oTexCoord ) * texture2D( Texture1, oTexCoord );\n"
	"	gl_FragColor = ColorBias + oColor * mix( movieColor, uiColor, uiColor.w * oColor.w );\n"
	"}\n";
...
void ShaderManager::OneTimeInit( const char * launchIntent )
{
	...
	LOG( "ShaderManager::OneTimeInit: %3.1f seconds", TimeInSeconds() - start );
}
